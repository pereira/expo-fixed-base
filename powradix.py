...
class PowRadix:
    def __init__(self, basis, k=1, n=None):
        ...
        self.k = k
        # now we start the precomputation
        table = []
        row_basis = basis
...
# Benchmarks
t1 = timeit('[pow(g, e, p) for e in e_list]', globals=globals(), number=10) * 100
print("Computing", n, "exponentiations with gmpy2: ", t1, "ms.")

t2 = timeit('PowRadix(g,n=n)', globals=globals(), number=10) * 100
print("Precomputation for radix method with k =", g_radix.k, ": ", t2, "ms.")

t3 = timeit('[g_radix.pow(e) for e in e_list]', globals=globals(), number=10) * 100
print("Computing", n, "exponentiations with radix method: ", t3, "ms.")

print("Speedup with radix method: ", t1/(t2+t3))

t4 = timeit('PowRadix2(g)', globals=globals(), number=10) * 100
print("Precomputation for radix 2 method: ", t4, "ms.")

t5 = timeit('[g_radix_2.pow(e) for e in e_list]', globals=globals(), number=10) * 100
